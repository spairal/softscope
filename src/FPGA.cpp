		...
		if(configuration.getMode() == Configuration::ROLL)
		{
			if(configuration.getChannel(Configuration::CHANNEL_A))
			{
				...
				if(N > 0)
				{
					...
					{
						samplesA[j + i] = configuration.getVerticalScaleValue(Configuration::CHANNEL_A) * (channelABuffer[j] - 128) / 32;
					}
