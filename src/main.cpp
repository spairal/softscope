...
class UserData
{
	...
	public:
		UserData(Configuration& configuration, State& state, Samples& samples) : touch(configuration, state, samples), fpga(configuration, samples), display(configuration, state, samples)
		{
		}
};
